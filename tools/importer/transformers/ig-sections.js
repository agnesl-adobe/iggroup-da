...
/**
 * Transformer: ig.com section breaks and section metadata.
 * Inserts an <hr> before every section except the first, and a Section Metadata
 * block after each styled section. Section definitions (selectors + styles) come
 * from page-templates.json template "home" (12 sections; styled: rc-trust-stats
 * = accent, rc-joinig = grey).
 *
 * Follows the reference implementation: breaks are inserted in beforeTransform
 * (while every section element still exists, before block parsers replace them),
 * using a marker attribute on the <hr> as a stable anchor; Section Metadata is
 * inserted in afterTransform anchored to that marker. Sections are processed in
 * reverse order so live-element inserts never disturb not-yet-processed sections.
 */
